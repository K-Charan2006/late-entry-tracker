        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <section className="space-y-6">
                <div className="glass-card p-8 bg-white border border-slate-100 rounded-3xl shadow-sm">
                    <h2 className="text-2xl font-bold mb-6">Student Lookup</h2>
                    <form onSubmit={handleSearchSubmit} className="space-y-4">
                        ...
                        <button
                            ...
                            className="w-full py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-lg"
                        >
                ...
                {student && (
                    ...
                    >
                        <div className="flex items-start justify-between mb-6">
                            <div>
                                <h3 className="text-3xl font-bold text-slate-900">{student.name}</h3>
                                <p className="text-slate-500 font-mono">{student.hallticket_id}</p>
                            </div>
                            <div className="bg-slate-100 px-3 py-1 rounded-full text-xs font-bold text-slate-600 uppercase">
                                Year {student.year}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 pt-6 border-t border-slate-100">
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-900/5 transition-all min-h-[100px] resize-none"
                                />
                            </div>
                            <button
                                onClick={handleLogSubmit}
                                disabled={loading || !reason}
                                className="w-full py-4 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 text-lg"
                            >
            ...
            {/* Recent Logs Section */}
            <section className="space-y-6">
                <div className="glass-card p-8 bg-white border border-slate-100 rounded-3xl shadow-sm h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                        {logs.length === 0 ? (
                            ...
                            logs.slice(0, 10).map((log) => (
                                <div key={log.id} className="p-4 rounded-xl bg-white border border-slate-100 hover:border-slate-200 transition-all group shadow-sm">
                                    <div className="flex items-center justify-between mb-2">
                                        <p className="font-bold text-slate-900">{log.name}</p>
                                        <p className="text-xs font-mono text-slate-400">{format(parseISO(log.timestamp), 'HH:mm')}</p>
                                    </div>
